...
"""
    ...
    try:
        ...
        return {"score": 50, "matched_skills": [], "missing_skills": [], "verdict": "Unable to parse response.", "recommendation": "Please try again."}
...
{{
  ...
  "target_score": 90,
  "gaps": [{{"skill": "skill name", "importance": "High/Medium/Low", "action": "what to do"}}],
...
"""
    ...
    try:
        ...
        return {"current_score": current_score, "target_score": 90, "gaps": [], "quick_wins": [], "timeline": "Unknown"}


async def rank_candidates(candidates: list, jd_text: str) -> list:
    results = []
    for c in candidates:
        prompt = f"""Rate this resume against the job description. Return ONLY JSON (no markdown):
RESUME:
{c.get('resume_text', '')}
JOB DESCRIPTION:
{jd_text}
...
async def executive_summary(resume_text: str) -> str:
    prompt = f"""Write a 2-sentence executive summary of this candidate for a recruiter.
RESUME:
{resume_text}
